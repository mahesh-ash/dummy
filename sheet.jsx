		...
		<SheetPortal>
			...
			<SheetPrimitive.Content
				...
				onOpenAutoFocus={(event) => {
					event.preventDefault(); //To Prevent autoFocus on sheet Open.
					const el = event.currentTarget;
					el.focus(); //To get Rid of console warning for aria-hidden
				}}
				...
				className={cn(
					...
					side === 'right' && 'data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right inset-y-0 right-0 h-full w-3/4 border-l sm:max-w-md',
					side === 'left' && 'data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left inset-y-0 left-0 h-full w-3/4 border-r sm:max-w-md',
...
function SheetHeader({ className, ...props }) {
	return <div data-slot="sheet-header" className={cn('flex flex-col gap-1.5 p-4 bg-[#EEE] sheet-header-bg dark:bg-[#2d3436]  border-b-1', className)} {...props} />;
}

function SheetFooter({ className, ...props }) {
	return <div data-slot="sheet-footer" className={cn('mt-auto flex flex-col gap-1.5 p-4 bg-[#EEE] sheet-header-bg dark:bg-[#2d3436] border-t-1', className)} {...props} />;
}
