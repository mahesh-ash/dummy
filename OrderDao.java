...
export default function ManageInvoiceTable(props) {
    ...
    try {
        toast.success('Downloading...');
        const response = await fetch(`${import.meta.env.VITE_API_URL}Attachment/Download?invoice_no=${encodeURIComponent(invoiceNo)}`, {
            method: 'GET',
            headers: {
                auth_token: encodeURIComponent(AUTH_TOKEN.value),
	...
	function createDataSource(fromDate = '', toDate = '') {
		return {
			getRows: async (requestParams) => {
				...
				// if (fromDate) {
				// 	formDataObj.start_date ="2025-11-10";
				// }
				// if (toDate) {
				// 	formDataObj.end_date = "2025-11-21";
				// }
				// if (requestParams.request.filterModel && Object.keys(requestParams.request.filterModel).length !== 0) {
					...
					formDataObj.order="";
				// }
                formDataObj.start_date ="2025-11-21";
				formDataObj.end_date = "2025-12-25";
	...
	function handleAddCoverage() {
		...
		const newRow = {
    emp_id: employeeData.emp_id || employeeData.id, // ADD THIS LINE
    ...
    if (getinvoiceDetailss.data && getinvoiceDetailss.isSuccess) {
        ...
        form.reset({
				...
				comments: rowData.comments || '',
				 att_id: rowData.att_id || 'existing_file', 
	...
	async function handleUpdateSubmit(data) {
      ...
      const filteredSplits = coverageRows.map((emp) => ({
			emp_id: String(emp.emp_id),
			split: parseFloat(emp.percentage).toFixed(5), // Sending as "split"
		}));
		// console.log(filteredSplits);
    ...
    // 3. Map values and convert to Strings for your API requirements
     const filteredSplits = coverageRows
        ...
        .map((emp) => ({
            // Safety: Ensure emp_id exists before calling toString
            emp_id: String(emp.emp_id), 
            // Ensure format matches backend 10.000
            split: parseFloat(emp.percentage).toFixed(5), 
        }));
													...
													{form.watch('att_id') ? (
														<div className="flex flex-col items-center justify-center w-full space-y-1">
															<div className="flex flex-col items-center text-center">
																<span className="text-xs sm:text-sm font-semibold text-blue-600 truncate max-w-[150px]">
																	{form.watch('invoiceFile')?.name || form.watch('att_name') || 'Attached Invoice'}
																</span>
																{/* Show size if available */}
																{(form.watch('invoiceFile') || form.watch('att_size')) && (
																	<span className="text-[10px] sm:text-xs text-gray-500">
																		({form.watch('invoiceFile') ? (form.watch('invoiceFile').size / 1024).toFixed(0) : form.watch('att_size')}{' '}
																		KB)
																	</span>
																)}
															</div>
															...
															<div className="flex gap-2 w-full max-w-[200px]">
																{editingRow && !form.watch('invoiceFile') && (
																	<Button
																		type="button"
																		variant="outline"
																		size="sm"
																...
																onBlur={(e) => {
																	...
																	if (!isNaN(val)) {
																		...
																		const updatedRows = coverageRows.map((r) =>
																			// FIX: Use the same unique key you used in onChange (preferably emp_id)
																			r.emp_id === row.emp_id ? { ...r, percentage: formattedVal } : r
																		);
																		setCoverageRows(updatedRows);
																	}
																}}
